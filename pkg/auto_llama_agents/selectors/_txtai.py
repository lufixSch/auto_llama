...
class SimilarityAgentSelector(AgentSelector):
    ...
    def __init__(
        ...
        self._tools = tools
        self._keywords = keywords
        self._none_keyword = none_keywords
    ...
    def run(self, chat, available_agents) -> AgentResponse:
        available_keywords = self._filter_agents(available_agents.keys(), self._keywords)
        keyword_mapping = KeywordMapping(assistant=self._none_keyword, **available_keywords)

        # prompt = self._coref_converter(chat)
        prompt = chat.last_from("user")
        similarities = ModelLoader.get("txtai.similarity", Similarity)(prompt, keyword_mapping.keywords_flat)

        keyword = keyword_mapping.keywords_flat[similarities[0][0]]
        tool = keyword_mapping.name(keyword)
